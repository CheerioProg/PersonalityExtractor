...
def predict_with_model(embeddings, l=0):
    ...
    for y in range(0, 5):
        ...
        if os.path.isfile('/content/drive/MyDrive/PersonalityExtractor/'+ model_file_name):
            classifier = load('/content/drive/MyDrive/PersonalityExtractor/'+ model_file_name)
        else:
             classifier, _, _, _ = svm.classify("/content/drive/MyDrive/PersonalityExtractor/essays_mairesse_sb_tokenized_200_max_rev_vector.p", y, -1, 0, add_mairesse=False)
...
if __name__ == "__main__":
    ...
    txt_input = text_input['text'].iloc[10001:15000]
    li = []
    for i in txt_input:
      x = i
